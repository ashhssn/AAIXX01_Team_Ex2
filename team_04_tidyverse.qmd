...
afghanistan_data <- data %>%
  filter(Country == "Singapore") %>%
  na.omit()

# Convert the 'dt' column to Date format and extract the year
afghanistan_data$Year <- as.numeric(format(as.Date(afghanistan_data$dt, format="%Y-%m-%d"), "%Y"))

# Average the data by unique year and round to 1 decimal place
yearly_data <- afghanistan_data %>%
  group_by(Year) %>%
  summarise(
...
scatter_plot <- ggplot(yearly_data, aes(x = Year, y = AvgTemperature)) +
  geom_point() +
  labs(
    title = "Scatter Plot of Year vs. Average Temperature Uncertainty",
    x = "Year",
    y = "Average Temperature Uncertainty (°C)",
    caption = "Data Source: [URL of your dataset]"
  ) +
  theme_minimal()
...
improved_scatter_plot <- ggplot(yearly_data, aes(x = Year, y = AvgTemperature)) +
  ...
  labs(
    title = "Improved Scatter Plot of Year vs. Average Temperature Uncertainty in Afghanistan",
    x = "Year",
    y = "Average Temperature Uncertainty (°C)",
    caption = "Data Source: [URL of your dataset]"
  ) +
...
scatter_plot_with_trend <- ggplot(yearly_data, aes(x = Year, y = AvgTemperature)) +
  ...
  labs(
    title = "Scatter Plot with Trend Curve of Year vs. Average Temperature Uncertainty in Afghanistan",
    x = "Year",
    y = "Average Temperature Uncertainty (°C)",
    caption = "Data Source: [URL of your dataset]"
  ) +
...
scatter_plot_with_loess <- ggplot(yearly_data, aes(x = Year, y = AvgTemperature)) +
  ...
  geom_smooth(method = "lm", se = FALSE, color = "red") +
  labs(
    title = "Average Temperature in Singapore (1825 - 2013)",
    x = "Year",
